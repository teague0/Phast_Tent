...
for(i in 1:length(unique(phast$tag_id))){
  plot(phast[[i]], col = 2)
  points(mv2 <- interpolateTime(phast[[i]], time=as.difftime(10, units="mins"), 
...
```{r}
df <- int[int$location_long < -82.336365,]

IDlocs <- table(df$tag_id)
i = 9
for(i in 1:length(names(IDlocs))){
  b <- df[df$tag_id == names(IDlocs)[i],]
  days <- unique(date(b$timestamp))
  layout(1:length(days))
  if(length(days) > 6){
    layout(1)
  }
  j = 1
  for(j in 1:length(days)){
    d <- b[date(b$timestamp) == days[j],]
  scan_track(time = d$timestamp, x = d$location_long, y = d$location_lat, 
...
df$ID <- paste0(df$local_identifier,"_", date(df$timestamp))
table(df$ID)
...
for(i in 1:length(IDs)){
  ...
  as.data.frame(with(coords, list(x = approx(x, n = N)$y, 
    y = approx(y, n = N)$y)))
