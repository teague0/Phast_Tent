...
IDlocs <- table(move.df$tag_id)
df <- move.df[move.df$tag_id %in% names(which(IDlocs>100)),]
...
# let's say we want the crawl-model to predict a relocation every 120 seconds / 2.5 minutes
...
timestamps <- dlply(df, 'local_identifier', function(id){
  tmp <- ddply(id, 'day', function(d){
    min.t <- min(d$timestamp)
    max.t <- max(d$timestamp)
...
df$ID <- df$local_identifier

tmp <- df[df$local_identifier=='7CCA9E6',]

test.crwl <- crawlWrap(tmp, Time.name='timestamp', coord=c('x', 'y'), predTime=timestamps[['7CCA9E6']],
...
```{r}
tmp <- test.crwl$crwPredict
ggplot(tmp[tmp$locType=='o',]) + geom_point(aes(x=x, y=y, colour=gps_hdop), size=0.7) + 
  labs(title='Observed') + coord_equal() + scale_colour_viridis_c(name='GPS accuracy [m]') +
  theme(legend.position='bottom') +
ggplot(tmp[tmp$locType=='p',]) + geom_point(aes(x=mu.x, y=mu.y, colour=se.mu.x), size=0.7) + 
...
```{r}
df.7CCA9E6 <- prepData(test.crwl, type='UTM')
...
hmm.ga
plot(hmm.ga, sepStates=T, plotTracks=F, plotCI=T, ask = FALSE)
```
### weibull
```{r}
...
```{r}
po <- spTransform(SpatialPoints(as.data.frame(crwl.all$crwPredict)[,c('mu.x', 'mu.y')], proj4string=projUTM), CRSobj=proj.ll)
crwl.all$crwPredict$solar.time <- unlist(lapply(1:nrow(crwl.all$crwPredict), function(j){
  noon <- solarnoon(po[j,], as.data.frame(crwl.all$crwPredict)$timestamp[j], POSIXct.out=TRUE)$time
  tod <- as.numeric(difftime(as.data.frame(crwl.all$crwPredict)$timestamp[j], noon, units='hours'))
  return(tod)
}))
# shift solar time to be all-positive, from noon to noon
crwl.all$crwPredict$solar.time <- (crwl.all$crwPredict$solar.time+24)%%24

crwl.all$crwPredict$day <- as.vector(unlist(dlply(as.data.frame(crwl.all$crwPredict), 'ID', function(id){
  c(0, cumsum(diff(id$solar.time)<0)) + 1
})))

# compute solar elevation
crwl.all$crwPredict$solar.pos <- unlist(lapply(1:nrow(crwl.all$crwPredict), function(j){
  pos <- solarpos(po[j,], crwl.all$crwPredict$timestamp[j])
  return(pos[,2])
}))

crwl.all$crwPredict$ID <- paste(crwl.all$crwPredict$ID, crwl.all$crwPredict$day, sep='-')

crwl.all$crwPredict

hmm.df <- prepData(crwl.all, type='UTM')
...
plot(ecdf(df$ground.speed), main='ECDF of ground speed (original data)')
```
