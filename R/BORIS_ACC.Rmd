...
# bolt$time <- dmy_hms(bolt$datetime)
# bolt$id <- "bolt"
# bolt$sampling <- 50
# 
# heart$time <- dmy_hms(heart$datetime)

star$time <- dmy_hms(star$timestamp)
# star$time0 <- round.POSIXt(star$time, units = "sec")
...
# load video data
```{r}
a <- data.table::fread("../../../../Dropbox/MPI/Dechmann_Lab/Johannes/boris_files/ExportedData/060422_1_1.csv", header = TRUE)
...
a06.1.1_bolt <- a06.1.1 %>% filter(Subject == "bolt")
...
ggplot(a06.1.1_heart_mlt, aes(x = start, 
                             ...
                             y = z+1, yend = z+1, col = factor(z)))+
  ylim(c(-4,4))+
  xlim(c(min(a06.1.1_heart_mlt$start)-1000, max(a06.1.1_heart_mlt$stop)+1000))+
  geom_segment(size = 2)+
...
a06.2.1_moon_mlt <- data.frame(start = a06.2.1_moon$Time[a06.2.1_moon$Status == "START"],
                               stop = a06.2.1_moon$Time[a06.2.1_moon$Status == "STOP"],
                               behavior = a06.2.1_moon$Behavior[a06.2.1_moon$Status == "START"])

ggplot(a06.2.1_moon_mlt, aes(x = start, xend = stop, y = behavior, yend = behavior, col = behavior))+ geom_segment(size = 2)
